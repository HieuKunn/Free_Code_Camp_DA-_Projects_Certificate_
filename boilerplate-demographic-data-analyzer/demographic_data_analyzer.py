...
def calculate_demographic_data(print_data=True):
    ...
    # average_age_men = round(df.loc[df['sex' == 'Male'], 'age'], 1)
    # -> df.loc[điều_kiện_chọn_hàng, tên_cột_muốn_lấy]

    # What is the percentage of people who have a Bachelor's degree?
    count_edu = df['education'].value_counts()
    percentage_bachelors = round((count_edu['Bachelors'] / len(df)) * 100, 1)
    ...
    rich_percentage = round(len(min_work_hours_df[min_work_hours_df['salary'] == '>50K']) / num_min_workers * 100, 1)

    # What country has the highest percentage of people that earn >50K?
    all_countries = df['native-country'].value_counts()
    rich_country_count = df[df['salary'] == '>50K']['native-country'].value_counts()
    country_rich_percentage = (rich_country_count / all_countries) * 100
    ...
    occupations = rich_india_df['occupation'].value_counts()
